...
class Application_Model_GpsContactoModel extends Zend_Db_Table_Abstract{
    ...
    public function Getbandejaid($id){
         try {
            ...
            $qry = $db->query("SELECT c.id, c.nombre, c.email_personal, c.mensaje, c.status_leido, c.status_imp, c.created_at, c.empresa, c.fecha_envio 
                        FROM contacto c
                        where c.id = ?",array($id));
            $row = $qry->fetchAll();
            $db->closeConnection();
    ...
    public function getbandejapag($table,$status_leido,$status_imp,$offset,$no_of_records_per_page){
        try{
            $db = Zend_Db_Table::getDefaultAdapter();
            $qry = $db->query("SELECT c.id, c.nombre, c.email_personal, c.mensaje, c.status_leido, c.status_imp, c.created_at, c.fecha_envio, c.empresa
						FROM contacto c
